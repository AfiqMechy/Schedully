...
class SchedullyApp {
    ...
    img.onload = () => {
      try {
        ...
        if (sorted.length > 0) {
          ...
          // Generate diverse, distinct, and harmonious course swatches from wallpaper
          const courseSwatches = [primaryHex];
          for (let i = 1; i < sorted.length && courseSwatches.length < 8; i++) {
            const rgb = parseRgb(sorted[i]);
            const hex = rgbToHex(rgb[0], rgb[1], rgb[2]);
            // Ensure visual distinction from already picked swatches
          ...
          // If wallpaper has limited color diversity, generate harmonious analogous/complementary accents
          if (courseSwatches.length < 6) {
            const [pr, pg, pb] = dominantRgb;
            ...
            const targetLum = Math.min(0.65, Math.max(0.42, l));
            const hueShifts = [40, 90, 150, 200, 260, 310];
            for (const shift of hueShifts) {
              if (courseSwatches.length >= 8) break;
              const newHue = (h + shift) % 360;
              const [nr, ng, nb] = hslToRgb(newHue, targetSat, targetLum);
              courseSwatches.push(rgbToHex(nr, ng, nb));
            }
          }
    ...
    const exportWallpaper = (onComplete) => {
      ...
      if (originalCanvas.classList.contains('canvas-tablet')) {
        ...
        if (originalCanvas.classList.contains('watch-shape-band') || ratio === 'android' || ratio === 'band') {
          nativeW = 194;
          nativeH = 368; // Smart Band Format (1:1.9)
        } else if (originalCanvas.classList.contains('watch-shape-capsule') || ratio === 'ios' || ratio === 'capsule') {
          nativeW = 192;
          nativeH = 490; // Pill Capsule Format (1:2.5)
        } else if (originalCanvas.classList.contains('watch-shape-round') || ratio === 'standard' || ratio === 'round') {
          nativeW = 466;
          nativeH = 466; // Round Circular Format (1:1)
        } else {
          nativeW = 410;
          nativeH = 502; // Squircle Format (4:5)
        }
      } else if (originalCanvas.classList.contains('canvas-story')) {
        nativeW = 380;
        nativeH = 675; // Exact 9:16 Social Story Format (1080 x 1920 Ultra-HD)
      } else if (originalCanvas.classList.contains('canvas-paper')) {
        nativeW = 720;
      ...
      const cloneClock = clone.querySelector('#phone-lock-header');
      const cloneTrademark = clone.querySelector('#lock-trademark-footer');
      const origTimetable = originalCanvas.querySelector('#lock-timetable-container');
      const origClock = originalCanvas.querySelector('#phone-lock-header');
      const origTrademark = originalCanvas.querySelector('#lock-trademark-footer');
      ...
      // Proportional Normalized Coordinate Engine for Phone & Tablet Modes
      if (!originalCanvas.classList.contains('canvas-paper')) {
        const isTab = originalCanvas.classList.contains('canvas-tablet');
        const previewH = originalCanvas.offsetHeight || (isTab ? 690 : 770);
        const vScale = nativeH / previewH;
        const topPadBase = isTab ? 20 : 14;

        clone.style.setProperty('padding-top', `${Math.round(topPadBase * vScale)}px`, 'important');

        if (origClock && cloneClock) {
          cloneClock.style.marginTop = `${Math.round(origClock.offsetTop * vScale)}px`;
        }

        if (origTimetable && timetableContainer) {
          const origTopPercent = origTimetable.offsetTop / previewH;
          const targetTopPx = Math.round(origTopPercent * nativeH);
          const clockBottomPx = cloneClock ? (cloneClock.offsetTop + cloneClock.offsetHeight) : 0;
          timetableContainer.style.marginTop = `${Math.max(0, targetTopPx - clockBottomPx)}px`;
        }

        if (origTrademark && cloneTrademark && this.showTrademark) {
          const origTradeTopPercent = origTrademark.offsetTop / previewH;
          const targetTradeTopPx = Math.round(origTradeTopPercent * nativeH);
          const tableBottomPx = timetableContainer ? (timetableContainer.offsetTop + timetableContainer.offsetHeight) : 0;
          cloneTrademark.style.marginTop = `${Math.max(10, targetTradeTopPx - tableBottomPx)}px`;
        }
      }
  ...
  renderWatchGlance(selectedDay = null) {
    ...
    // Apply accent glow to watch clock in real time
    const lockTime = document.getElementById('lock-time');
    if (lockTime && this.activeDevice === 'watch') {
    ...
    displayedClasses.forEach((c, idx) => {
        ...
        if (this.axisMode === 'period') {
          ...
          if (mode === 'both') {
            timeSubHtml = `
              ...
              <span style="font-size: 5px; opacity: 0.6; line-height: 1;">-</span>
          ...
          if (mode === 'both') {
            timeContentHtml = `
              ...
              <span style="font-size: 6px; opacity: 0.6; line-height: 1; margin: 0.5px 0;">to</span>
      ...
      if (this.globalCourseType !== false && c.type) {
        ...
        const iconSvg = isSeminar
          ? `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`
          : `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>`;
      ...
      if (this.globalCourseRoom !== false && c.room) {
        ...
        const venueIcon = isOnline
          ? `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>`
